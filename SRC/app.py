    ...
    with FileLock(lock_path, timeout=5):
        ...
        if counter_file.exists():
            try:
                ...
                raise RuntimeError(f"Corrupt alert_counter.json — cannot allocate Ref No safely. Delete {counter_file} and retry.")
            
        # Write updated counter atomically (temp file → os.replace)
        temp_path = counter_file.with_suffix('.json.tmp')
        with open(temp_path, 'w') as f:
            json.dump({'date': date_str, 'count': current_count}, f)
            f.flush()
            os.fsync(f.fileno())
        os.replace(str(temp_path), str(counter_file))
...
with streamlit_analytics.track(unsafe_password=st.secrets.get("analytics", {}).get("password", "")):
    ...
    with st.container(border=True):
        ...
        if st.button("💻 กดเพื่อประเมินความเสี่ยง (Run PRAISE Risk Assessment)", use_container_width=True):
            if user_price is None:
                ...
                if risk_eval.get('action_code') is not None:
                    risk_eval['action_code'] = str(risk_eval['action_code'])
            
                alert_ref = None
                if risk_eval['is_anomaly'] and risk_eval.get('action_code', '') != '890':
    ...
    if alert_info and alert_info.get('profile_tuple') == selected_tuple and alert_info.get('eval', {}).get('is_anomaly'):
        ...
        action_code = eval_res.get('action_code', '')
        if action_code == '88':
    ...
    with tab2:
        ...
        if alert_info and alert_info.get('profile_tuple') == selected_tuple:
            ...
            if eval_res.get('action_code', '') == '90':
            ...
            elif eval_res.get('action_code', '') == '890':
                st.markdown(f"""
                <div style="background-color:#fff3cd;color:black;padding:20px;border-radius:10px;border: 2px solid #ffeeba;">
                ...
                if eval_res.get('action_code', '') == '89':
